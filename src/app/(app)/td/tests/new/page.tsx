...
export default async function TDNewTestPage({ searchParams }: Props) {
  try {
    ...
    const { eventId, tournamentId, trialEventName, trialEventDivision: _trialEventDivision } = await searchParams
    ...
    // Ensure staffMembershipId is defined before rendering
    if (!staffMembershipId) {
      console.error('Failed to create or find staff membership')
      redirect('/td')
    }
    ...
    console.error('Error in TDNewTestPage:', error)
    redirect('/td')
  }
}
